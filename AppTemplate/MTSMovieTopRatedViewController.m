//
//  MTSMovieTopRatedViewController.m
//  AppTemplate
//
//  Created by MonsterSucker on 14/06/2017.
...
@interface MTSMovieTopRatedViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@implementation MTSMovieTopRatedViewController
...
- (void)viewDidLoad {
    ...
    _collectionView.contentInset = UIEdgeInsetsMake(kScreenHeight * 0.175, 0, 0, 0);
    _collectionView.scrollIndicatorInsets = UIEdgeInsetsMake(kScreenHeight * 0.175, 0, 0, 0);
...
- (void)requestNewData {
    [[JLTMDbClient sharedAPIInstance] GET:kJLTMDbMovieTopRated withParameters:@{@"page" : @(_currentPage)} andResponseBlock:^(id response, NSError *error) {
        if (response) {
